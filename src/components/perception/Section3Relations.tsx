...
export const Section3Relations: React.FC = () => {
  const { register, formState: { errors } } = useFormContext();
  const section = perceptionSections.find((s) => s.id === 3)!;
              ...
              <div className="mt-2 grid gap-3 sm:grid-cols-2">
                {q.options?.map((opt) => (
                  <label key={opt.value} className="flex cursor-pointer items-center gap-2 text-white/90">
                    <input type="checkbox" value={opt.value} {...register(q.id, { required: q.required })} className="accent-[#d9c34b]" />
                    <span>{opt.label}</span>
                  </label>
                ))}
              </div>
              {errors[q.id] && <p className="mt-2 text-sm text-red-400">Sélectionne au moins une option.</p>}
            </div>
          );
        }
        if (q.type === "radio") {
...
export default Section3Relations;
