          ...
          <div key={q.id} className="rounded-xl border border-white/10 bg-white/5 p-4 backdrop-blur">
            ...
            <Input
              ...
              {...register(q.id, {
                required: q.required,
                pattern: isEmail
                  ? { value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, message: "Email invalide." }
                  : isWhatsapp
                  ? { value: /^\+213[0-9]{9}$/, message: "Format attendu: +213XXXXXXXXX" }
                  : undefined,
              })}
            />
            <p className="mt-1 text-xs text-white/60">{q.helperText}</p>
            {errors[q.id]?.message && (
              <p className="mt-2 text-sm text-red-400">{String(errors[q.id]?.message)}</p>
            )}
            {!errors[q.id]?.message && errors[q.id] && (
